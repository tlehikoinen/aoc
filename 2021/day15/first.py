...
costs = [[1e9 for y in range(len(data[0]))] for x in range(len(data))]
...
while not pq_non_visited.empty():
    ...
    for i in [[1,0],[0,1],[0,-1],[-1,0]]:
        ...
        if nx not in range(0, len(data)) or ny not in range(0,len(data[0])):
            continue
        # Update costs if it is less than currently known 
        if costs[x][y] + data[nx][ny] < costs[nx][ny]:
            costs[nx][ny] = costs[x][y] + data[nx][ny] 

        # Put the neighbour into non_visited to be visited later
        pq_non_visited.put((data[nx][ny] + costs[x][y],nx,ny))
...
# Part 1 answer
print(f'Part 1 answer = {costs[len(data)-1][len(data[0])-1]}')
