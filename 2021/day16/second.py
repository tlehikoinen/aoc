...
expressions = [
        ...
        min, 
        max,
        lambda values : 1 if values[0] > values[1] else 0,
        lambda values : 1 if values[0] < values[1] else 0,
        lambda values : 1 if values[0] == values[1] else 0
        ]
...
def read_packet(index):
    ...
    if type_id == 4: # Literal version
        ...
        value += convert_bin_dec(new_bin)
        #print(f' Literal value {new_dec}')

    else: # Operator versions
        # No expression for literal version
        expression = expressions[type_id] if type_id < 4 else expressions[type_id-1]
        values = []
...
index, value = read_packet(0)
print(f'Part 2 result {value}')
